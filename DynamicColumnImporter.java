 ...
 * This file is a modified version of https://github.com/apache/hbase/blob/master/hbase-server/src/main/java/org/apache/hadoop/hbase/mapreduce/TsvImporterMapper.java ; 
 * The file has been modified to accomodate reading column key from the import file instead of the input parameters. 
 * 
...
public class DynamicColumnImporter
...
//          tags.add(new ArrayBackedTag(TagType.TTL_TAG_TYPE, Bytes.toBytes(ttl)));
 ...
 //       }
      ...
      if(parsed.getColumnCount()==3)
      {
        // Assuming that if column count is 3 , rowKey is first column, columnQualifier is second & third column is value
        // This is specific for GE Healthcare
        populatePut(lineBytes,parsed,put,2,true);
      ...
      else
      {
          for (int i = 0; i < parsed.getColumnCount(); i++) {
            ...
            populatePut(lineBytes, parsed, put, i,false);
  ...
  protected void populatePut(byte[] lineBytes, ImportTsv.TsvParser.ParsedLine parsed, Put put,
      ...
      int i,boolean isDynamic) throws BadTsvLineException, IOException {
    ...
    if(isDynamic)
    {
        if (hfileOutPath == null) {
          ...
          // Replacing getQualifier with lineBytes, parsed.getColumnOffset(i-1), parsed.getColumnLength(i-1); ie picking up the qualifier from the file.
          // Change made by @shwetams on 17th March, 2017

        cell = new KeyValue(lineBytes, parsed.getRowKeyOffset(), parsed.getRowKeyLength(),

            parser.getFamily(i), 0, parser.getFamily(i).length, lineBytes, parsed.getColumnOffset(i-1),

            parsed.getColumnLength(i-1), ts, KeyValue.Type.Put, lineBytes,
        ...
        cell = this.kvCreator.create(lineBytes, parsed.getRowKeyOffset(), parsed.getRowKeyLength(),
            ...
            parsed.getColumnLength(i-1), ts, lineBytes, parsed.getColumnOffset(i),
